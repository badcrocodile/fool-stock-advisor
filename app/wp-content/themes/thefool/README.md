...
## Local Installation using Lando:
...
**Installation:**
1. [Download or clone the repository](https://github.com/badcrocodile/fool-stock-advisor): `git clone git@github.com:badcrocodile/fool-stock-advisor.git`
1. Fire up your terminal and cd into the newly created directory: `cd /path/to/project/`
1. Set up your server with Lando by running vagrant up: `lando start`
1. Grab some coffee while the server provisions.
1. Once the script is done you can visit your site by visiting [http://foolish-stock-advisor.lndo.site/](http://foolish-stock-advisor.lndo.site/)
1. Follow the onscreen prompts to install WordPress
1. Activate the Advanced Custom Fields plugin to support the site functionality.
...
1. Configure the site sidebar in **Appearance -> Widgets** to contain only the Category widget. 
1. **OPTIONAL:** If you'd like to start with some relevant seed data, locate the `afoolishproject.WordPress.xml` file located in the theme directory and use the WordPress importer to load this content.

## Installation from Scratch
If you'd rather install the files into an existing WP installation here's what you need to do. 

1. [Download the repository](https://github.com/badcrocodile/foolish-project).
1. Locate the _Foolish Stock Advisor_ theme directory at `/wp-content/themes/thefool/` and move it into your installation.
1. Locate the _Foolish Company Data_ plugin directory at `/wp-content/plugins/fool-company-data/` and move it to your installation.
1. Locate the _Foolish Shortcodes_ plugin directory at `/wp-content/plugins/fool-shortcodes` and move it to your installation.
...
**Steps to validate:**
- The easy way: 
  - Navigate to [_Starbucks Reports Record Quarter, but Challenges Remain_](http://foolish-stock-advisor.lndo.site//2018/07/starbucks-reports-record-quarter-but-challenges-remain/)
...
**Steps to Validate:**
- The easy way:
    - Navigate to [_Buy Starbucks (SBUX)_](http://foolish-stock-advisor.lndo.site/2021/04/buy-starbucks/)
    - Notice that authors name, publish date, and article content are all displayed.
    - Click the "Edit Post" link in the WP Admin bar to validate the requirement of writing and editing the article.
...
**Steps to Validate:**
- Navigate to the [Stock Recommendation archive page](http://foolish-stock-advisor.lndo.site/category/stock-recommendation/)
...
**Steps to Validate:**
- The easy way:
    - Navigate to the [Starbucks Company Page](http://foolish-stock-advisor.lndo.site/company/sbux/)
...
- There are logs tracking API errors at `/wp-content/plugins/fool-company-data/logs/`. I left mine in the repo if you're interested.
